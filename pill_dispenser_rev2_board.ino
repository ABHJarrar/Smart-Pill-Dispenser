...
char server[] = [VPS address]; // VPS IP
...
int device_id = 2145;               //device's product ID
...
void wait_for_app_connection();        // Prompt the user to connect the device to the app
...
void display_time();               // Continuously update LCD with current time and next pill time
...
void rotate_motor();             // Dispense a pill by rotating the motor until cup is detected
...
void setup() {
  ...
  wait_for_app_connection();        //Wait for user to connect via app
  //print out the connection data-------------------------
  ...
  while(state == 'C'){
    if( command == pill_1){
      ...
      if( command ==  cancel || command == disconnecting){
        display_time(); // Return to time display if cancelled
      }else if( command == set){
...
void wait_for_wifi_connection() {
  // Kick off the connection (make sure SECRET_SSID / PASS are set)
  ...
  // Once we’re here, we’re connected:
  lcd.clear();
  lcd.setCursor(0, 0);
 ...
 */
void wait_for_app_connection(void){
  char reading=0;
  lcd.clear();
  lcd.setCursor(1,0);
    ...
    if(memory[hours] == memory[counter+1]){         //the following three if conditions to compare currently time 'hours, minutes and AM or PM' to
      if(memory[minutes] == memory[counter+2]){     //the next pill that's ready to be dispensed
        if( memory[AM_PM] == memory[counter+3]){
          ...
          rotate_motor();                        //calling stepper moving function to rotate the motor.
...
/*
 * The following 2 functions are related to rotating the stepper motor.
 */

// The rotate_motor function will incorporate the Hall sensor's state.
...
void pollForSchedules() {
  client.get("/api/wait-for-schedule");
  client.get("/api/device_id");
  client.get("/api/deleted_schedule");
  int status = client.responseStatusCode();
  if (status == 200) {
    String body = client.responseBody();
    client.post("/api/device_id");
    client.post("/api/pill_dispensed");
...
/*
 * interrupt service routine of timer2 overflow interrupt.
 */
ISR(TIMER2_OVF_vect){
  TCA0.SINGLE.CNT = 0;       //clear the counter
  counter++;
